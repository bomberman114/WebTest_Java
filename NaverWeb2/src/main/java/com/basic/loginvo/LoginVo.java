...
import com.basic.login.api.LoginApi;
...
public class LoginVo {
	...
	private final static String CLIENT_ID = "4e15HS5U0FSmLjgyOOgG";
	private final static String CLIENT_SECRET = "6M5hAIjx7l";
	...
	public OAuth2AccessToken getAccessToken(HttpSession session, String code, String state) throws IOException {
		...
		if (StringUtils.pathEquals(sessionState, state)) {
			...
			OAuth20Service oauthService = new ServiceBuilder().apiKey(CLIENT_ID).apiSecret(CLIENT_SECRET)
					.callback(REDIRECT_URI).state(state).build(LoginApi.instance());
	...
	public String getUserProfile(OAuth2AccessToken oauthToken) throws IOException {
		...
		OAuth20Service oauthService = new ServiceBuilder().apiKey(CLIENT_ID).apiSecret(CLIENT_SECRET)
				.callback(REDIRECT_URI).build(LoginApi.instance());
